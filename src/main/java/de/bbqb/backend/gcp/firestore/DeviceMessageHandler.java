...
public class DeviceMessageHandler implements MessageHandler {
	 ...
	 */
	@Value("${bbq.backend.gcp.pubsub.message-header-deviceid}") //TODO: Move all @Value annotation to spring context configuration class and inject values from there
	...
	public void handleMessage(Message<?> message) throws MessagingException {
		LOGGER.info("Message arrived! Payload: " + new String((byte[]) message.getPayload()));
			...
			deviceRepo.findById(deviceId).flatMap(device -> {
				...
				originalMessage.ack();
			}); // If no Device is found do nothing. In such cases the message is not acknowledged
	}
